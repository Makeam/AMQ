...
feature 'User sign in with Vkontakte' do
  ...
  context 'Email does not provided.' do
  ...
  context 'After the confirmation e-mail' do
    let!(:authorization){ create(:vkontakte_authorization, user: registered_user) }
    scenario 'User re-login after the confirmation e-mail' do
      visit new_user_session_path
      omni_auth :vkontakte
      click_on 'Sign in with Vkontakte'
